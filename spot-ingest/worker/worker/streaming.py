...
'''
    Listen a particular topic of the Kafka cluster and write incoming records to a Hive
table, via Spark2 Streaming module.
'''
...
def parse_args():
    ...
    parser   = ArgumentParser(
        ...
        usage='worker [OPTIONS]... -d <dbname> -p <partition> -t <pipeline> '
            '--topic <topic> -z zk_server:zk_port')
    ...
    required.add_argument('-d', '--database-name', required=True, metavar='STRING',
    ...
    required.add_argument('-z', '--zkquorum', required=True, metavar='STRING',
        help='connection string for the zookeeper in the form \'host[:port]\'')
